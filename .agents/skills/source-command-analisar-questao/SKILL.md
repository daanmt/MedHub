...
10. **Custo é métrica -- e a fonte é o HARNESS:** cada spawn registra **tokens + minutos** no selo da sessão, lidos do `usage` que o harness devolve no resultado do spawn (tokens totais e duração). É o que torna esta régua falsificável -- sem o número, ela vira intenção. 🔴 **O brief NÃO pede custo ao filho (s189, `/ai-eng`):** custo auto-relatado é o instrumento relatando sobre si mesmo -- na s188 um filho declarou 35-40 min e o harness mediu 8,5 (mesma regra da cláusula 7: número só com a fonte que o mede). ⚰️ *Revogado em 19/09/2026: pedir ao filho, no brief, os próprios tokens/minutos.*
