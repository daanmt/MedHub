...
| `analises/<lista>_<num>` | hub (após `/analisar-questao`); página grava o veredito | `lista`, `num`, `pedia`, `cadeia[]` (vazia desde a s200: usa a cadeia da Solução v2), `quebrou` (índice 0-based na cadeia), `nao_usou` (s200: elos 0-based que ele declarou ter e que excluíam a letra marcada -- a página pinta de âmbar, "sabia, não usou"; verde é só o que as riscadas provam), `comporta`, `armadilha`, `veredito_hub`, `cards[]`, `questao_erro_id`; `veredito_operador` (`concordo` · `em_parte` · `discordo`), `nota_operador`, `veredito_em` |
