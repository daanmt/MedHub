...
4. **Decidir:** `python tools/hub.py --precisa-publicar --lote tmp/player_<sessao>.json --notas tmp/player_<sessao>_db --quadro-estado tmp/hub_db` -> `sim|nao`, a `acao` e o motivo:
   ...
   - **`mesmo_lote`** (lote em curso, mas o painel ou o quadro mudou) -> passo 7 com o **MESMO** `tmp/player_<sessao>.json`: mesmo `sessao`, mesmos cards; as notas já dadas voltam do `db` no reload do celular. A linha 3 do HANDOFF **não** muda. 🔴 Nunca trocar lote em curso: o reload reapresentaria a fila no meio do drill.  <!-- CHECK: test_painel_mudou_com_lote_em_curso_republica_o_mesmo_lote -->
   - **`nova_fila`** (lote drenado, ou vazio) -> passos 5-7.
   - **Aula feita com tarefa pendente** (linha `aula feita no quadro com tarefa pendente: #N`): `python tools/plano.py --concluir N --leitura` (decisão do operador, s194: o "feito" do quadro É a confirmação de leitura). O CLI recusa tarefa com lista ou questões previstas -- recusa = relatar no session log e seguir, nunca forçar com `--sessao` inventado. Concluir muda o painel: o próximo tique republica.  <!-- NAO-VERIFICAVEL: a recusa do CLI e testada; 'relatar e nunca forcar com --sessao inventado' e conduta do tique, sem artefato que a registre (revisar: 2027-03-31) -->
...
6. **Próxima fila:** `fsrs_queue.py --export-player --sessao <AAAA-MM-DD><letra>` -- data de hoje + a próxima letra livre (`2026-09-23a`, `b`...; nunca reusar um `sessao`, é o nome da coleção). Sem `--limit`: o export corta no SALDO do dia (teto menos as revisões já gravadas hoje -- `consumo_hoje` na saída). **Fila de véspera** (saldo de hoje zerado e ele vai drenar de manhã sem o PC): `--export-player --para <amanhã> --sessao <amanhã>a` -- o relógio anda para as 06:00 de amanhã (buckets, teto, consumo 0, previsões), NUNCA `--limit` de véspera: o lote de hoje-à-noite não vê o que vence amanhã e a agenda da tela de fim sai deslocada (F131). A saída traz `retidos_reforja` (cards com defeito marcado que a fila segura até a reforja, F132). `total` 0 = saldo zerado: se o lote da aba também está vazio, apagar o export e seguir com o lote **atual** (vazio) -- publicar só se o `--precisa-publicar` tinha apontado painel ou quadro mudado; senão, no-op. No dia seguinte o saldo volta e o 1o tique publica a fila nova.  <!-- NAO-VERIFICAVEL: o --para e testado (F131); 'nunca reusar sessao' e 'nunca --limit de vespera' sao escolha de flag no ato, sem artefato que a registre (revisar: 2027-03-31) -->
...
**Declaração de capabilities do hub (s194, com o quadro; s197, com a aba Questões):** `{db: {rules: [{path: "", read: "view", write: "admin"}, {path: "sessoes", write: "interact"}, {path: "quadro", write: "interact"}, {path: "listas", read: "admin", write: "admin"}, {path: "questoes", read: "admin", write: "admin"}, {path: "respostas", read: "admin", write: "admin"}, {path: "analises", read: "admin", write: "admin"}]}}` -- as 4 coleções do EMED só o dono lê (o hub é compartilhado por link). O tique **nunca** a passa (publica sem `capabilities`, que mantém a declarada); quem a publica é o agente principal, uma vez. Se o publish do tique voltar sem a regra `quadro` (o controle "feito" desabilitado no hub), o tique relata -- não declara.  <!-- NAO-VERIFICAVEL: 'o tique publica sem capabilities' e conduta no ato do publish; nenhum gate le a chamada do Artifact (revisar: 2027-03-31) -->
...
- **Permissão:** cada tique publica e grava FSRS sem o operador olhando. Se uma chamada pedir aprovação ou o classificador barrar, o tique **para e relata** -- nunca contorna.  <!-- NAO-VERIFICAVEL: parar e relatar diante do classificador e conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->
