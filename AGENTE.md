...
**Este projeto é uma jornada contínua.** Nunca comece do zero. Sua missão é herdar o estado da sessão anterior, executar a tarefa atual e preparar o terreno para a próxima.  <!-- NAO-NORMATIVA: principio/doutrina de abertura, nao prescricao operavel -->
...
🔴 **Proporcionalidade de delegação (F93, s175/s176):** decidir *sozinho* é o default; decidir **delegar a um subagente** tem régua própria e versionada -- as 10 cláusulas em [`.claude/commands/analisar-questao.md §0 Orquestração`](.claude/commands/analisar-questao.md). Ela nasceu na análise de questão mas **vale para toda delegação** (varredura de engenharia, auditoria de docs, curadoria de cards): até ~8 itens o principal faz sozinho; um subagente só por lote; nunca cadeia de subagentes; `model` explícito; retorno destilado <= 3k + arquivo; número do filho só vale com o comando que o produziu; custo (tokens + minutos, **lidos do `usage` do harness -- nunca o que o filho declara**, s189) no selo da sessão. Este ponteiro existe porque a régua morava num portador que só é lido em tarefa de questão -- **regra certa que não alcança** é o mesmo defeito do F90 um nível acima.  <!-- NAO-VERIFICAVEL: decisao de delegar acontece ANTES de existir artefato; conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->
...
A obtenção de aprovação integral (status `✅ PASSED` com exit code `0`) é critério inegociável de *Definition of Done*. É proibido transferir ao usuário o ônus de rodar linters ou detectar regressões visuais e de código.  <!-- NAO-VERIFICAVEL: 🔴 RAIO ALTO, custo declarado -- o harness e rodavel, mas nada obriga o agente a roda-lo ANTES de reportar; o gate exigiria amarrar 'reportar conclusao' a um artefato, e nao ha evento de conclusao (revisar: 2027-03-31) -->
...
4. **Plano do Dia** -- o hook `SessionStart` já rodou `tools/day_plan.py` e injetou o plano antes do 1o turno: **não re-rodar**. Liderar com ele e propor o passo imediato. **A abertura entrega o PANORAMA ao operador (s190, pedido dele em 20/09/2026)** -- meta e datas, onde estamos, tarefas em aberto com o que fazer em cada (lista / caderno a criar / aula-base / sem lista), passos de hoje com a cota, métricas -- montado só dos blocos derivados que o hook injeta (Plano do Dia inteiro + `plano.py --panorama`); a ordem das seções mora no texto-contrato de `tools/hooks/memory_boot.py`, preso por `test_boot_entrega_o_panorama`. Sob demanda: `--difficulty`, `--tempo H --energia alta|media|baixa`. Ritmo-alvo mede a **grade**, nunca a prova (`core/provas.json`, countdown no cabeçalho). O bloco `🧭 Cronograma` sai de **`plano_tarefas`** (semana do plano, fase, próximas tarefas com link e nº previsto); conclusão/ordem são colunas editáveis por `tools/plano.py`, ⚰️ *não mais pela W8/`--sync-drive`, revogados em 17/09/2026*. Normas: `forgetting-curve`/`orquestracao`/`cronograma`-contract. Pausas: §1.1.  <!-- NAO-VERIFICAVEL: conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->
...
4. **Acúmulo de conhecimento** — armadilhas são **cumulativas**; novos insights se somam aos antigos, nunca substituem.  <!-- NAO-VERIFICAVEL: acumulo exige comparar versoes e `audit_resumos` audita 1 versao; candidato a ratchet de contagem (revisar: 2026-12-31) -->
5. **Convenção de Encoding e Zero LaTeX (sessão 103/108):** Jamais utilizar sintaxe de LaTeX inline (`$ ... $` ou `$$ ... $$`), comandos matemáticos (`\rightarrow`, `\le`, `\ge`, `\mu`), ou cifrões encapsulando números e desigualdades (`$< 60$`, `$> 1000$`, `$\rightarrow$`) em resumos, flashcards, logs ou respostas no chat. É terminantemente proibido o uso de setas Unicode (→), aspas ou travessões inteligentes (–, —). Usar exclusivamente Markdown e ASCII limpo: seta simples (`->`), sinais de menor/maior diretos (`< 60`, `> 1000`, `<=`, `>=`), aspas retas normais (' ou ") e hifens simples/duplos (- ou --), garantindo legibilidade perfeita e zero quebra visual em qualquer terminal ou app externo. 🔴 **Esta regra governa PONTUACAO e NOTACAO, jamais ORTOGRAFIA (F113, s185):** portugues acentuado e obrigatorio no texto clinico de resumos e flashcards (`criança`, `síndrome`, `função`, `após`, `não`) -- "ASCII limpo" se refere aos caracteres especiais enumerados acima, nunca aos acentos. Ler a frase como "tire os acentos" produziu 875 de 1.476 cards (59,3%) com ortografia quebrada, medido em 17/09/2026; o operador os leu como erro de portugues.  <!-- CHECK: test_audit_resumos -->
...
- **SSOT volumétrica** = `sessoes_bulk` no `ipub.db`. Ao informar "fiz X questões, acertei Y", o agente DEVE chamar `python tools/registrar_sessao_bulk.py --sessao NNN --area AREA --feitas X --acertos Y` ANTES de processar erros individuais.  <!-- NAO-VERIFICAVEL: 🔴 RAIO ALTO, custo declarado -- o writer existe e e testado, mas 'chamar ANTES de processar erros' e ordem de atos numa conversa; o F38 pega o RESULTADO (bloco com erro e sem linha), nao a ordem (revisar: 2027-03-31) -->
...
- **Retenção de PDF para RAG (sessão 086 -- reverte o Zero PDF)** -- os PDFs-fonte do **EMED** são **mantidos** dentro de `resumos/` na taxonomia EMED (ex.: `resumos/GO/2. Planejamento Familiar.pdf`) como **insumo de AUTORIA** do `.md`. ⚰️ *A justificativa original desta linha -- "pois serão usados para alimentar o RAG" -- foi **revogada de fato** pela consolidação part-2, que tornou o RAG **gold-only** (a decisão duas linhas acima: `pdf_raw` e `search_two_tier()` removidos; `index_resumos.py` indexa `.md`). Lápide escrita em 10/09/2026 na triagem do F17 -- a premissa morta contradizia, dentro do mesmo §6, a decisão que a matou.* São **IP do EMED** -> **gitignored** (`.gitignore` cobre `*.pdf`/`*.PDF`), nunca commitar. Fluxo: extrair texto (`PyPDF2`), cunhar/reformar o `.md` conforme `/estilo-resumo`, **deixar o PDF no lugar** (não deletar). Resumo `.md` recebe o nome-tema do EMED **sem prefixo numérico**; o EMED vincula banco↔cronograma↔nome, então a taxonomia EMED é preservada. A skill `/extrair-pdf` (delete-after-extract) está desatualizada quanto à deleção.  <!-- NAO-VERIFICAVEL: politica de retencao de IP; conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->
- **Regra de Acúmulo** -- armadilhas de prova são cumulativas; jamais sobrescrever, apenas somar.  <!-- NAO-VERIFICAVEL: mesmo vao da linha 79 -- sem comparacao de versoes nao ha sensor (revisar: 2026-12-31) -->
- **Camada de estado contract-driven (sessão 075)** -- estado vive em duas camadas: `HANDOFF.md` (operacional curto, ≤60 linhas, lido primeiro) + `ESTADO.md` (macro). Normatizado por `core/contracts/{handoff,estado,reconcile,fsrs-management}-contract.md`. Padrão adaptado do agente irmão `agente-daktus-content`. Boot roda check de reconcile; fechamento atualiza HANDOFF sempre.  <!-- CHECK: test_handoff_teto -->
...
- **Governança de evidência (sessão 076)** -- afirmação clínica decisória (conduta/dose/cutoff/critério) é auditada contra a melhor evidência: hierarquia **sociedades BR + MS > RCT/meta + guidelines INT > consenso**, com **lente da banca** (o que ENAMED/ENARE espera). Conflito banca × evidência atual -> ensina a resposta da banca **e** registra 🔴 armadilha "banca-dependente" (nunca silenciar). Substrato: **PubMed MCP do plugin `pubmed@life-sciences`** (server `plugin_pubmed_PubMed`; verbatim por PMID/DOI, com texto integral via PMC) + WebSearch (sociedades BR em PDF) + RAG local (`app/engine/rag.py`). O server antigo `pubmedmcp` do `.mcp.json` morreu (`CONNECTION_CLOSED`) e foi substituído na s169 -- contrato em v1.1. Normatizado por `core/contracts/evidence-governance.md`; operado por `/pesquisar-evidencia` + subagente `evidence-researcher`. Adaptado do mecanismo de auditoria do `agente-daktus-content`. Escopo v1.0: go-forward + sob demanda (sem varredura retroativa).  <!-- NAO-VERIFICAVEL: hierarquia de evidencia e julgamento semantico; conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->
- **Cards de altura graduada / andaime de pré-requisito (sessão 082)** -- a altura de um flashcard é um **gradiente** (`base -> mecanismo -> nuance -> topo`), carregado no campo `tipo`. Cards de andaime (altura < topo) reconstroem os elos **a montante** quando um **CLUSTER** de cards-alvo trava por falta de grounding (card isolado caindo = recall, não falta de base). Nascem **sem erro de origem** (`questao_id=NULL`), ancorados no resumo, via `tools/insert_card_base.py`. **Propagação local:** tapar o buraco costurando o degrau imediatamente adjacente; o nº de degraus é inferido da iteração com o estudante. O degrau `mecanismo` (porquê causal encadeado) é o de maior rendimento -- o gap costuma ser **causalidade, não fato**. Calibração de compressão na revisão (**reescrita na s170**): a dose de fundação entra na **Revisão Direcionada de fechamento** e escala com o quão frio está o tema (stability + acerto do cluster) -- ver `/revisar`, Cláusula 11 do `revisao-calibrada-contract`. ⚰️ O gatilho *pré-bloco* foi revogado com a Camada 0; o de andaime sobrevive, disparado **pelo resultado do drill**. Régua de autoria em `.claude/commands/estilo-flashcard.md`. **Schema formal pendente (Tier 3):** altura ordinal + grafo `prereq_de` + ordenação automática da fila base->topo. Adaptado dos princípios de `ai-eng` (grounding, subcategory targeting, velocidade > perfeição).  <!-- NAO-NORMATIVA: descreve o gradiente de altura, nao prescreve conduta -->
- **Gestão da curva de esquecimento (sessão 083)** -- o MedHub gere a curva **no nível do TEMA** (o FSRS gere no card). Ritual diário `/refrescar` (`tools/dormant_refresh.py`): seleciona o tema mais dormente (`tools/review_radar.py`), re-ensina em prosa narrativa **menos comprimida** (substrato via `app.engine.get_topic_context`) e carimba em **`review_log`** -- o **SSOT do tempo-de-revisão temática**. 🔴 **Fronteira dura:** o refresh **NÃO toca o FSRS** (não chama `record_review`, não cunha card). Boot **proativo** (§2 passo 4, `tools/day_plan.py`) cruza dormência × volume × FSRS × cronograma e lidera com plano decidido. **Invariante anti-poluição:** identidade do tema = `(area, tema)` com `UNIQUE` em `taxonomia_cronograma` (a dedup de s083 colapsou 22 grupos via `tools/dedup_taxonomia.py`, merge MAX; `insert_questao`/`insert_card_base` resolvem por `(area,tema)`). Normatizado por `core/contracts/forgetting-curve-contract.md`.  <!-- CHECK: test_invariante_a -->
- **Sync do cronograma (sessão 095)** -- o cronograma de Reta Final (EMED) tem **SSOT = `Cronograma.pdf`** (gitignored, IP) derivado para **`core/cronograma/grade.json`** (versionado, estrutural) por um **derivador único** `tools/cronograma.py`. 🔴 **Fronteira dura:** read-only no `ipub.db` -- **zero write** em `taxonomia_cronograma`/`sessoes_bulk`/FSRS/`review_log` (o elo cronograma↔desempenho é em memória, join por `AREA_PDF_TO_CANON`); a **única exceção** é a tabela **`plano_tarefas`** (v1.3, 17/09/2026), com os writers nomeados na Cláusula 5 -- ⚰️ *era "o único write da feature é o ponteiro textual `Próxima = SNN`", morto junto do ramo calendário do `day_plan`*. Plano não é verdade-de-estado -> reconcile W5-W7/W5b nunca BLOCKING. `day_plan.py` lê o PLANO; `cronograma.py` segue derivando o PDF e semeando. Normatizado por `core/contracts/cronograma-contract.md`; skill `/cronograma`.  <!-- CHECK: test_writer_allowlist -->
- **Harness autônomo staged-only + warning-first (sessão 106/107)** -- `tools/auto_check.py` valida antes do commit: o git pre-commit hook roda `--staged` (audita só o que será selado, quotepath-safe p/ caminhos acentuados), e o Reflexo Autônomo §1.3 roda `--changed` (árvore). Achados têm **duas severidades**: **BLOCK** (exit 1, bloqueia — Armadilhas ausente, tabela ASCII, `UnicodeDecodeError`) e **WARN** (exit 0, só adverte, agregado por tipo — frontmatter §5.2 incompleto, encoding não-ASCII proibido, drift de paridade command↔skill). Regra nova **nasce WARN** e só vira BLOCK "quando a base zerar". Normatizado pelas specs `autogovernanca-proativa-part-2/3`.  <!-- CHECK: test_linter_escopo_staged -->
- **Fonte canônica de skills = `.claude/commands` + espelhos gerados (sessão 107)** -- `.claude/commands/*.md` é a **fonte canônica única** das skills; os `.agents/skills/source-command-*/SKILL.md` são **build artifacts** gerados por `tools/sync_skills.py` (nunca editados à mão). `sync_skills --check` (consumido pelo `auto_check`) acusa quando alguém edita o canônico sem regenerar o espelho (WARN de paridade, não bloqueia). Estende a disciplina §7.2 ("assinatura canônica em UMA skill") ao par command↔skill.  <!-- CHECK: test_espelho_gerado -->
- **Load balancing do agendamento FSRS (sessão 128)** -- o FSRS agenda cada card isoladamente (`due = hoje + I`) e ninguém olha o calendário, o que produz carga **grumosa**: 48 cards num dia ao lado de dias com 2. Como a curva de retenção é ~plana numa vizinhança pequena de `I`, existe folga de graça: `app/utils/fsrs_balance.py` escolhe, **dentro dessa folga**, o dia de **menor carga já agendada**. 🔴 **Fronteiras duras:** (a) **não toca `stability` nem `difficulty`** -- move apenas a DATA; (b) janela conservadora de **±5% do intervalo** (piso 1 dia, teto 10); (c) só card de **revisão (`state == 2`) com intervalo >= 4 dias** -- passos de aprendizado/relearning são intra-sessão e mover 1 dia num card de 2 dias é erro de 50%, não folga; (d) **nunca agenda no passado ou hoje**; (e) empate preserva o dia que o FSRS calculou (só desvia com ganho real); (f) **calendário de provas (F71, s174):** o blackout (dia da prova + o seguinte, lido de `core/provas.json` -- nunca data no código) não recebe card e não é cruzado; alvo em blackout vai para ANTES da prova e, sem vaga na folga, fica onde está e vira OVERFLOW declarado (`tools/fsrs_load.py --blackout` re-roda o balanceador sobre a fila com dry-run + COUNT-ASSERT). O módulo é **puro** (recebe a carga como dict, não importa `sqlite3`) -- a consulta vive em `db.carga_agendada`, respeitando a regra de SSOT, e por isso a regra é testável sem banco. Ponto de aplicação: dentro de `record_review`, o **caminho único de escrita do FSRS**; falha no balanceamento nunca derruba a gravação da revisão (degrada para o `due` original com WARN). Visibilidade: `tools/fsrs_load.py` mostra a distribuição e o **CV** (desvio/média) que o achatamento deve derrubar. Suíte `tools/test_fsrs_balance.py` é **BLOCKING** no `auto_check` (check 2c) -- regressão aqui corrompe a curva.  <!-- CHECK: test_fsrs_balance -->
- **Revisão Calibrada (sessão 096; v1.3 na s170; v1.4 na s183)** -- `/revisar` é a **competência única** de revisão. **Duas fases, nesta ordem:** **DRENAR** (card-a-card no chat **ou** no player de cards em Artifact -- Cláusula 12, s183; a página nunca grava FSRS, o `--record-lote` grava por `record_review`) e **REVISÃO DIRECIONADA de fechamento** (o **único** lugar onde a sessão ensina, ancorada nos temas de **nota 1-2**). ⚰️ **O sub-modo PREPARAR, a Camada 0/1 e o Invariante D foram REVOGADOS na s170** por decisão do usuário: o aquecimento pré-drill não era lido ("muito ruim, denso e confuso") e os quatro nomes -- refrescar/PREPARAR/Revisão Direcionada/aula-base -- colapsaram num vocabulário confuso. **Não re-derivar** (Cláusula 11 do contrato). A `/aula-base` NÃO foi afetada: ela é pré-**QUESTÕES**, não pré-cards. Descompressão calibrada por uma **nota de dificuldade 1-10** por tema (`taxonomia_cronograma.dificuldade`, via `db.set_dificuldade`; escopo de escrita real na allowlist testada em `tools/test_writer_allowlist.py`), mapeada a 4 degraus (D10/D8/D5/D2) -- a nota calibra a **Revisão Direcionada**. Inferência determinística `infer_nota()` (`tools/day_plan.py --difficulty`, read-only). 🔴 **Invariante A:** o ensino nunca escreve FSRS. 🔴 **Invariante B:** toda Revisão Direcionada carimba `review_log`. 🔴 **Invariante F (s170):** silêncio no meio do DRENAR -- nota e tally, prosa só no fechamento; exceção única é **defeito de card**. Normatizado por `core/contracts/revisao-calibrada-contract.md`.  <!-- CHECK: test_invariante_a -->
- **Fundamentos pedagógicos nomeados (sessão 184, 2026-09-17)** -- cada mecanismo de ensino do MedHub (FSRS, curva por tema, question-first, Autópsia, aula-base, Revisão Direcionada, teto 60/dia, player) mapeia a um **princípio nomeado da ciência da aprendizagem** com fonte verificada em [`docs/FUNDAMENTOS-APRENDIZAGEM.md`](docs/FUNDAMENTOS-APRENDIZAGEM.md) (efeito de teste, espaçamento, intercalação, dificuldades desejáveis, reaprendizagem sucessiva, pré-teste/productive failure, Mayer). O arquivo é **explicativo, não normativo** (a norma segue nos contratos e skills), e carrega o **ledger de fricções** -- quais atritos do estudo são virtuosos (protegidos) e quais são viciosos (automatizar). 🔴 Decisão nova de ensino ou reforma de "reduzir atrito": conferir o princípio e o ledger de fricções lá **antes** de re-derivar (F109-F112). Plano de reformas derivado: `PLANEJAMENTO-APRENDIZAGEM-2026-09-17.md` (raiz).
...
- **Skills (`.claude/commands/*.md`) são referência atômica.** Especificam protocolo, assinatura de CLI, padrão de estilo, template de resposta. Não contêm orquestração de chamadas de CLI (sequência "rode X, depois Y" é do workflow). Protocolo cognitivo interno numerado (etapas de análise, loop conversacional) é permitido -- cláusula explicitada na s166 (drift D4: 5 de 12 skills já numeravam protocolo de raciocínio).  <!-- NAO-VERIFICAVEL: 'nao conter orquestracao' e semantico; o D5 cobre so a assinatura de CLI; conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->
- **Workflows (`.agents/workflows/*.md`) são orquestração imperativa.** Numeram passos, invocam skills por nome/path (`.claude/commands/<skill>.md`), mas nunca reespecificam o conteúdo das skills. Quando um workflow precisa de detalhe de CLI, regra de estilo ou protocolo de análise, ele referencia a skill e termina ali.  <!-- NAO-VERIFICAVEL: mesmo vao semantico da linha 193 (revisar: 2027-03-31) -->
- **Cada CLI em `tools/` tem assinatura canônica em UMA skill.** A assinatura completa (todos os flags, semântica de cada argumento) vive em exatamente um `.claude/commands/*.md`. Workflows referenciam por nome de skill + seção; jamais copiam a invocação.  <!-- CHECK: test_cli_assinatura -->
...
🔴 **A terceira regra passou a ter instrumento (D5, s177).** Ela viveu em prosa desde que foi escrita, e prosa não acusa: medido em 11/09/2026, **65 das 168 flags (39%) não apareciam em skill nenhuma** e **17 CLIs não eram citados por skill alguma** — inclusive `recurate_cards` (reescreve o baralho in-place) e `normalize_taxonomia` (destrutivo). Agora `tools/cli_signature_check.py` mede, e o `auto_check` **BLOQUEIA**: flag declarada por `add_argument` e ausente de toda skill que cita aquele arquivo é achado. A base foi zerada no mesmo commit — CLI sem skill de estudo passou a ter casa em [`/engenharia-cli`](.claude/commands/engenharia-cli.md). Limites do sensor (extração estática; mede presença, não semântica; flag genérica pode colar em skill vizinha e **inflar** a cobertura) estão declarados na docstring do módulo, nunca maquiados.  <!-- NAO-NORMATIVA: narrativa da medicao que originou o D5 -->
...
| `/revisar` | **Competência única de revisão (s096; reescrita na s170)** -- duas fases: **DRENAR** (player FSRS card-a-card; nota e tally, **sem prosa no meio** -- Invariante F) e **REVISÃO DIRECIONADA de fechamento** (a única superfície de ensino da sessão, sobre os temas de nota **1-2**; FSRS read-only; carimba `review_log`). ⚰️ O sub-modo **PREPARAR** e a Camada 1 foram **revogados na s170**. CLIs: `fsrs_queue.py` (inclui `--export-player`/`--build-player`/`--record-lote`, s183), `dormant_refresh.py`, `day_plan.py --difficulty`. Norma: `revisao-calibrada-contract.md` v1.4 |
| `/refrescar` | **[DEPRECADO s096]** fundido em `/revisar`; desde a **s170** o re-ensino de tema dormente acontece na **Revisão Direcionada de fechamento**, não antes do drill. Stub de redirecionamento; o CLI `dormant_refresh.py` (com `--kind`) segue servindo o carimbo |
...
| `/engenharia-cli` | **(s177, D5)** Assinatura canônica dos CLIs de **engenharia** — sensores do harness, manutenção do `ipub.db`, curadoria em lote, `day_plan.py` e `preparacao.py`. É a casa dos CLIs que não pertencem a nenhuma skill de estudo; sem ela a §7.2 era inexequível para 17 deles |
| `/banco-emed` | **(s197)** Banco de questões dentro do MedHub (⚰️ *a captura pelo Chrome saiu em 26/09/2026, F135: o caderno entra em PDF por `tools/prova_pdf.py`, s201*): a **Bancada EMED** (artifact privado, `db`) foi o buffer da captura, o operador (aba Resolver: letra + confiança `solida/duvida/chute` ANTES do gabarito, racional e elo declarados no erro) e o hub (`tools/emed_banco.py` importa para `emed_questoes`/`emed_respostas`, analisa por `/analisar-questao` e devolve `analises/*` com veredito do operador). 🔒 Conteúdo EMED nunca no hub público nem no git |  <!-- NAO-VERIFICAVEL: IP do EMED; conduta do agente, sem gate que leia o que foi publicado (revisar: 2027-03-31) -->

### 7.4 CLIs ativos (`tools/`)

> **Tabela GERADA — não editar à mão.** Regenerar com `python tools/reachability_check.py --tabela` e colar aqui.
> Antes da consolidação part-6 esta tabela era mantida à mão e listava 21 dos CLIs vivos; o check de alcançabilidade encontrou os demais. Uma tabela digitada à mão envelhece em silêncio — que é exatamente o defeito (*construído-e-nunca-conectado*, achado D4) que o check existe para pegar.  <!-- NAO-NORMATIVA: justificativa do desenho da tabela gerada -->
...
| `tools/backup_db.py` | Backup do ipub.db com integrity_check + rotacao keep-5 EMBUTIDA | `.agents/skills/source-command-engenharia-cli/SKILL.md`, `.agents/workflows/curar-cards.md` (+8) |
...
| `tools/cards_prune.py` | poda de flashcards APOSENTADOS do ipub.db [DESTRUTIVO], dry-run por default | `.agents/skills/source-command-engenharia-cli/SKILL.md`, `.agents/skills/source-command-revisar/SKILL.md` (+9) |
| `tools/cards_regen_queue.py` | fila de regeneração de flashcards em JSON | `.agents/skills/source-command-estilo-flashcard/SKILL.md`, `.claude/commands/estilo-flashcard.md` (+2) |
| `tools/cards_rendimento.py` | o card que CONSOME revisao e nao retem (s187) | `.agents/skills/source-command-engenharia-cli/SKILL.md`, `.claude/commands/engenharia-cli.md` (+3) |
...
| `tools/dedup_taxonomia.py` | colapsa linhas duplicadas (area,tema) em taxonomia_cronograma | `.agents/skills/source-command-engenharia-cli/SKILL.md`, `.agents/workflows/curar-cards.md` (+5) |
...
| `tools/emed_banco.py` | banco de questões EMED no ipub.db: ingere questões e respostas da Bancada EMED, poda o b | `.agents/skills/source-command-banco-emed/SKILL.md`, `.claude/commands/banco-emed.md` (+7) |
...
| `tools/hub.py` | monta o MedHub HUB: UMA pagina (Cards + Aulas + Painel) e o manifesto do publish | `.agents/skills/source-command-aula-base/SKILL.md`, `.agents/skills/source-command-engenharia-cli/SKILL.md` (+28) |
...
| `tools/listas.py` | ledger de LISTAS de exercicios: quais listas do plano ja foram feitas, | `.agents/skills/source-command-engenharia-cli/SKILL.md`, `.agents/skills/source-command-importar-planilha/SKILL.md` (+21) |
...
| `tools/plano.py` | o plano de estudo como DADO (`plano_tarefas`): semeadura, listagem e | `.agents/skills/source-command-banco-emed/SKILL.md`, `.agents/skills/source-command-cronograma/SKILL.md` (+59) |
...
| `tools/prova_pdf.py` | caderno de prova em PDF -> docs `questoes/*` do hub (caminho PDF, s201) | `.agents/skills/source-command-banco-emed/SKILL.md`, `.claude/commands/banco-emed.md` (+2) |
...
| `tools/reforja.py` | a fila de reforja de flashcards como ESTADO consultavel (B2, s176) | `.agents/skills/source-command-analisar-questao/SKILL.md`, `.agents/skills/source-command-engenharia-cli/SKILL.md` (+41) |
...
- `error_count` vem de `ipub.db` por match **exato** do par (area, tema); par sem correspondência fica em 0 -- nunca herda o total da área.  <!-- CHECK: test_memory_counter -->
...
3. **Quem edita skill roda o sync.** `.claude/commands/<slug>.md` é o canônico; depois de
   editar, `python tools/sync_skills.py` e `--check` (exit 0) **no mesmo commit**. Nunca  <!-- CHECK: test_espelho_gerado -->
...
6. **Protocolo de achado com o `/ai-eng` (s170; divisão revista na s171).** 🔄 **Decisão do
   operador em 08/09:** **implement E audit são do MedHub** — o audit roda pelo **loop do
   vibeflow** (`/vibeflow:audit` sobre o trace doc ou o spec: DoD binário, Critical Gate,
   PASS/PARTIAL/FAIL). O `/ai-eng` fica na **orquestração**: triagem de remédio, sequência,
   GO/NO-GO/ALTERA nas bifurcações. ⚰️ *Era "audit é do `/ai-eng`, entre parts"* — mudou porque
   quem tem o contexto do repo e o loop de DoD é quem deve verificar, e o ida-e-volta deixava as  <!-- NAO-NORMATIVA: narrativa da decisao de divisao com o /ai-eng -->
   duas janelas ociosas. **Silêncio do `/ai-eng` = GO para seguir**; espera-se resposta dele só
   em decisão (ALTERA/NO-GO) ou pergunta. Dúvida NOVA de escopo: perguntar antes de escrever.
   Quando
...
7. **Operação em massa sobre o `ipub.db` (s170).** Nunca `UPDATE` direto: passa pelos **writers**  <!-- CHECK: test_writer_allowlist -->
   com `card_checks` (allowlist testada em `tools/test_writer_allowlist.py`, F49). Toda operação
   em lote exige **COUNT-ASSERT + dry-run escritos antes de executar** — o número esperado de  <!-- NAO-VERIFICAVEL: os CLIs de lote implementam COUNT-ASSERT, mas 'escrito ANTES de executar' e ordem de atos; conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->
...
8. **Gate que não pega vira registro, não silêncio (s170).** Achado de **leitura humana** que um
   ...
   semântico), ele é **declarado como não-verificado** — nunca convertido numa métrica inventada  <!-- NAO-VERIFICAVEL: meta-regra sobre honestidade de metrica; conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->
...
9. **Memória da auditoria (s171).** Antes de retomar qualquer frente de engenharia com o
   ...
   **§11 do mesmo arquivo (s174) é o inventário de dívida técnica em 4 tiers** — a fila progressiva
   que o `/ai-eng` ordena e o MedHub executa; item fechado ganha lápide, nunca sai da tabela.  <!-- NAO-VERIFICAVEL: o G14/G14b pega STATUS divergente, nao remocao de linha; conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->

10. **Revogar uma cláusula tem TRÊS passos, e os três vão no MESMO commit (F90, s175).** (1) **declarar**  <!-- CHECK: test_contrato_revogado -->
   a revogação no contrato/ledger; (2) **lapidar** a cláusula morta no portador que o agente *lê no ato*  <!-- NAO-NORMATIVA: continuacao da clausula da linha 374 -->
   ...
   que alguém lembrou de cadastrar. Declarar sem lapidar deixa o agente obedecendo cláusula morta em uso  <!-- NAO-NORMATIVA: continuacao da clausula da linha 374 -->
   ...
   é **declarado**, nunca maquiado (§10.8, *verification-stack*). A troca da enumeração manual por  <!-- NAO-VERIFICAVEL: meta-regra (declarar o limite do gate); conduta do agente, sem artefato que a registre (revisar: 2027-03-31) -->
