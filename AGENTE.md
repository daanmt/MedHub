...
4. **Plano do Dia (proativo)** -- após o reconcile passar, **liderar com um plano decidido** (não um menu): rodar `python tools/day_plan.py` e apresentar (≤8 linhas) -- tema dormente do dia (`/refrescar`), volume vs ritmo-alvo (~94q/dia p/ ENAMED), fila FSRS (vencidos + backlog) e próximo tema do cronograma -- e **propor o passo imediato**. Pausar só em fork real / operação destrutiva sobre SSOT / fronteira de PR (ver §1.1). Governado por `core/contracts/forgetting-curve-contract.md`. **Recomendação do dia (orquestração):** o day_plan emite o mix recomendado (mini-drill/questões/FSRS/simulado/descanso) com justificativa e projeção; com contexto do dia, rodar `python tools/day_plan.py --tempo H --energia alta|media|baixa`. Norma: `core/contracts/orquestracao-contract.md`. **Sync de conclusão real + ordem do cronograma (W8) -- AÇÃO OBRIGATÓRIA quando STALE:** o snapshot do Drive (`preparacao_estado.cronograma_conclusao_drive`) carrega tanto o que já foi **concluído** (tema riscado) quanto a **ordem real** das tarefas por semana (o usuário reordena o xlsx à mão -- ver `project_cronograma_dual_ssot`). Quando o day_plan sinaliza `⚠️ Drive desatualizado`, buscar o xlsx `Cronograma de Reta Final.xlsx` via MCP Drive (`download_file_content`, fileId em `.claude/commands/importar-planilha.md`), salvar os bytes decodificados em arquivo temporário e rodar `python tools/cronograma.py --sync-drive <path>` **antes de apresentar os `próximos temas`** -- isso é passo **obrigatório** do boot, não opcional; 1× por dia-calendário basta (o snapshot cacheia em `preparacao_estado`, reusado nos boots seguintes do mesmo dia). Sem snapshot fresco, os `próximos temas` podem vir **fora de ordem ou com temas já feitos**; por isso, se o MCP falhar/estiver indisponível (sessão headless), apresentar o plano calendário-only **com caveat explícito nomeando esse risco** (temas possivelmente já concluídos / fora da ordem do usuário), **nunca em silêncio**. Nunca bloqueia o boot (Cláusula 6). Norma: `core/contracts/cronograma-contract.md` Cláusula 5 + `reconcile-contract.md` W8.
...
| `tools/auto_check.py` | Harness autônomo: orquestra linter + suítes (`--changed`/`--staged`/`--all`); relatório BLOCK (bloqueia) vs WARN (adverte). Pre-commit hook usa `--staged` |
| `tools/setup_hooks.py` | Instala/restaura o git pre-commit hook que roda `auto_check --staged` (staged-only + quotepath-safe) |
| `tools/sync_skills.py` | Gerador determinístico `.claude/commands/*.md` → `.agents/skills/source-command-*/SKILL.md` (fonte canônica única); `--check` acusa drift de paridade (WARN no auto_check) |
