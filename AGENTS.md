...
- `tools/auto_check.py` roda no git hook pre-commit (`--staged`). Cerca de 20 checks nomeados, selecionados dinamicamente pelos arquivos tocados. Gates BLOCKING: linter de resumos (críticos), suíte pytest completa, suítes de FSRS/card quando o arquivo tocado é relevante, ponteiro de sessão (B2) e teto do HANDOFF (B1); o resto é WARN agregado. Matriz condição->instrumento em `core/contracts/reconcile-contract.md`.
