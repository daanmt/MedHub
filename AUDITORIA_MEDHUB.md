...
### F137 -- a SUITE fazia backup REAL do `ipub.db` a cada rodada (`backup_db.py --help` sem argparse, chamado pela varredura de CLIs) e a rotacao keep-5 APAGOU os pontos de retorno do dia -- **ALTA** -- **PARCIAL (s201: causa corrigida -- `--help` sem efeito, rotacao pelo nome, propriedade na varredura; spec do /ai-eng parte 1 feita -- `--fixar` + sha256; parte 2 ABERTA -- amarrar ao ato destrutivo, `--desfixar --motivo`, propriedade da rotacao nos writers; o dano e irreversivel)**
