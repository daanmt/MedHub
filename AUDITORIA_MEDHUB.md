...
### F135 -- captura pelo Claude Code dirigindo o Chrome MARCOU uma alternativa na conta real do operador no EMED (t65, s199): clique por coordenada para fechar o painel de solucao, 2x -- **ALTA** -- **PARCIAL (s201: caminho do Chrome fora do fluxo por decisao do operador em 26/09; revogacao formal na skill e o caminho PDF = item 6 da s201)**
...
### F134 -- LATENTE: `db._emed_ler` transformava coluna ausente em `[]` silencioso; um banco nao migrado faria o `--exportar` semear o hub SEM nenhuma Solucao MedHub -- **MEDIA** -- **ABERTO (s201: fix no codigo desde a s200 sem teste; hotfix com teste de PERTURBACAO e o item 2 da s201)**
...
### F133 -- GATE-MISS: a pagina gravava `riscadas` nos docs `respostas/*` desde a s197 e `db.emed_upsert_respostas` as DESCARTAVA (sem coluna e sem `extras`); nenhum teste pegou -- **MEDIA** -- **PARCIAL (s200: coluna + leitura; s201: falta o teste de PROPRIEDADE que pega a classe)**
