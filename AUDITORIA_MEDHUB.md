...
### F98 -- a guarda de blackout do F71 nao cobre intervalo CURTO, que e exatamente o que a sessao produz -- **MEDIA** -- **ABERTO**
...
### F99 -- nao existe CLI que sirva um card por ID: o "proximo passo" do HANDOFF nao era executavel -- **BAIXA/MEDIA** -- **ABERTO**
...
### F100 -- re-ensinar a materia NAO fechou o gap: 3 pontos ensinados na s175 falharam 2x na s179 -- **ALTA** -- **ABERTO (achado de metodo, nao de codigo)**
...
### F101 -- pendencia FANTASMA no HANDOFF: item copiado por 2 sessoes sem ninguem medir o que ele afirmava -- **MEDIA** -- **ABERTO (so-dado + candidato a check)**
...
### F102 -- o hook de boot acusa "drift de estado" por REGEX case-sensitive: falso positivo em toda sessao cujo texto cite uma sessao anterior antes do cabecalho -- **BAIXA** -- **ABERTO (hotfix de 1 linha, aguardando permit)**
...
### F103 -- `[SEM-LASTRO]` FALSO por nome de tema: "Polipos e Neoplasias Intestinais" tem resumo sob outro titulo -- **MEDIA** -- **ABERTO (so-dado; alias e candidato)**
...
### F104 -- resumo em linguagem COLOQUIAL/dramatica (`Neurologia/TCE.md`) + stub generico duplicado (`[CIR] TCE.md`, 17 linhas) -- **BAIXA/MEDIA** -- **ABERTO (conteudo)**
...
### F105 -- 6 perguntas COMPOSTAS em cards antigos (#570, #682, #685, #686, #691, #599) chegaram ao drill sem passar pela reforja, apesar do WARN de atomicidade -- **MEDIA** -- **ABERTO (so-dado; 11 marcas abertas)**
...
### F109 -- premissa "revisao em cluster e pedagogicamente superior" (F3, s108; PRD `engenharia-ledger-f1-f13`) contradita pela literatura de intercalacao e pelo perfil de erro do usuario -- **BAIXA/MEDIA** -- **ABERTO (so-dado + 2 riders de doc na fila)**
...
- **Fila (nao executado):** rider no `fsrs-management-contract.md` §Politica de fila ("ordem natural intercalada = default deliberado; `--cluster` = onboarding de cluster frio/andaime") + 1 linha em `revisar.md` passo 1; ambos `doc`, mesmo commit, `sync_skills` obrigatorio.

### F110 -- as friccoes VIRTUOSAS do estudo (recall antes do verso, recall a frio, nota honesta, relearning ate criterio, racional declarado, ritual de prova, triagem humana) nao tinham portador que as declarasse como protegidas -- **MEDIA** -- **ABERTO (portador criado; gate impossivel, declarado)**
...
### F112 -- a regua de notas 1-4 do agente esta deslocada um degrau em relacao a semantica do py-fsrs: nota 2 ("recall parcial sem o alvo") e agendada como ACERTO (Hard) e nota 4 (acerto normal) recebe o bonus de Easy -- **ALTA** -- **ABERTO (spec; toca `record_review`)**
...
### F113 -- cards cunhados SEM acentuacao (ASCII) sao lidos pelo usuario como "erro de portugues"; a convencao de encoding (AGENTE §4.5) foi aplicada ao TEXTO CLINICO do card, nao so a pontuacao -- **MEDIA** -- **ABERTO (curadoria; feedback do usuario no player, 17/09)**
