...
- **Erros & Cards:** **364 erros; 386 cards ativos (41 andaimes)**. **Curadoria s097:** triagem dos 409 ativos (workflow 20 agentes) → 335 ok / 37 reforjados (cunhagem 10 agentes, ancorados no elo) / 29 fundidos / 6 atomizados; taxonomia saneada 115→98. Pipeline `insert_questao.py` (+ `insert_card_base.py`/`insert_card_extra.py`); curadoria via `recurate_cards.py` + `detect_clones.py` + `normalize_taxonomia.py`. Backlog FSRS: vencidos + ~290 novos.
