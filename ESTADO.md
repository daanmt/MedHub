...
*Atualizado: 2026-07-09 (sessão 115 -- engenharia: PRD `boot-cronograma-drive-confiavel`, costura headless/Drive fechada) | Ferramenta: Claude Code (Fable 5)*
...
- **Indicador Atual:** **4.865** -- **48,7% da meta-prova (10.000)** · 40,5% do teto (12.000). Detalhe por sessão vive em `history/` (**s115** engenharia -- PRD `boot-cronograma-drive-confiavel` fecha a costura headless/Drive, F30/F31 RESOLVIDO + F34 no ledger; **s114**, 09/07 -- S12 avança 2/6; marco ENAMED corrigido nos scripts p/ 10k meta-prova + 12k teto; achado dos **dois SSOTs do cronograma** -- ordem no xlsx do Drive vs detalhamento no `Cronograma.pdf`, memória `project_cronograma_dual_ssot`). Padrões de erro vivos e pendências operacionais no `HANDOFF.md`; contadores macro abaixo.
...
- **Contadores:** **63 resumos** em `resumos/` (s110p1: MFC, Vulvovaginites blindados; s110p2: Imunizações expandido, Pré-Natal criado do zero) · **443 erros** em `ipub.db` (+10 na s110p2: 751-761) · **465 cards ativos** · **taxonomia 98 temas** (`UNIQUE(area,tema)`, áreas 100% canônicas, + colunas `dificuldade*`) · **RAG two-tier:** gold 1.048 chunks + `pdf_raw` 14.216 chunks (data/chroma, local-only)
...
- **Volume & Metas:** **4.865 / 10.000 meta-prova (48,7%)** [teto 12.000]. Perf. geral **~79,4%**. ENAMED 13/09: 66 dias, ~77,8q/dia p/ o alvo de 10k (recalculado s114 -- marco corrigido de 12k pra 10k nos scripts). Cluster fraco (volume + %): Oftalmo, Dermato, Cardiologia, Otorrino, Nefrologia, Hemato. **Gargalo nº1 = execução de prova** (bug nº1: não fechar a conduta / exame lido por dado parcial / ancoragem no fármaco).
- **Conteúdo:** **63 resumos** (s103-105: +Quadril Pediátrico, Polipose/CCR, Síndrome do Olho Vermelho; s101: Imunizações, Sepse, Anemias Hemolíticas). **Gaps:** reescrever `TCE.md` + `Sistemas de Informação em Saúde.md`; ectópica/icterícia neonatal (só andaime). PDFs EMED p/ aula-base mantidos (gitignored).
