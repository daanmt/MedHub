...
*Atualizado: 2026-06-27 22:17 (sessão 094) | Ferramenta: Claude Code (Opus 4.8)*
...
- **Indicador Atual:** **4.112** — **41% da meta-prova (10.000)** · 34% do teto (12.000) — **+126 na s094** (fecha a Semana 10: 5 blocos / 126q/87 = 69% — Hepatites 88% · Pneumo Int. 80% · Diabetes Comp.Crônicas 76% · Meningites Rev 70% · Ácido-Base 26%). Perf. geral 79,4%.
...
- **Contadores:** **51 resumos** em `resumos/` (+4 na s094) · **359 erros** em `ipub.db` · **~389 cards ativos** (41 andaimes; #588 último — **flashcards do dia pendentes**) · **taxonomia `UNIQUE(area,tema)`**
- **Cronograma (SSOT `Cronograma.pdf`, 28 semanas — 2 de revisão removidas):** Semana 09 ✓ · **Semana 10 ✓ FECHADA — 11/11 tasks (273q):** s092 107 + s093 40 + s094 126 (Meningites Rev · Diabetes Comp. Crônicas · Ácido-Base · Pneumo Intensiva · Hepatites Virais). **Próxima = Semana 11.** Calendário na S13 (~3 sem atrás). Frentes ativas: sync do cronograma (`docs/plans/s094-ultraplan.md`) + **PRD de Revisão Calibrada** (`docs/plans/s094-revisao-calibrada-PRD.md`).
...
- **Erros & Cards:** **359 erros; ~389 cards ativos (41 andaimes)** (#588 último). **Flashcards do dia da s094 ainda a cunhar (s095).** Pipeline `insert_questao.py` (+ `insert_card_base.py` andaime); taxonomia `UNIQUE(area,tema)`. Backlog FSRS: #554-588 + vencidos.
...
5. **Implementar (s095+):** **flashcards do dia da s094** (a cunhar); **F1 do sync** (`tools/cronograma.py` + `core/cronograma/grade.json`) — inexistentes, **desbloqueiam o PRD de Revisão Calibrada**. **Pendentes:** Tier-3 (schema de altura), limpeza `[bulk]`/`Geral` da taxonomia, enumeração do Drive, re-drill dos 1 (426, 435, 84, 78, 70, 104), sessão dedicada de Cirurgia, integrar `/schedule` no calendário.
