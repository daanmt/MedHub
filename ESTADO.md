...
- **Infraestrutura:** contratos em `core/contracts/`; harness `auto_check`; watermark de dado nos writers; `core/provas.json` (multi-prova). **s160-s161: ciclo DES-COLAGEM entregue pelo /ai-eng** (PRD `descolar-motor-determinismo`, 7 specs, P1-P7; F45-F61 resolvidos; suite 317->358; audits PASS 7/7). **s162: reforma VALIDADA em uso real** -- a s161 rodou inteira em harness estrangeiro (Antigravity) bootando so do repo, com F58/F60 acusando e o painel de DIVIDA abrindo o `posicao_ssot` no minuto do selo. Novos abertos: **F63** (prioridade roxa do cronograma nao viaja com o repo), **F64** (gatilho do regime de divida le `atrasados`, o dono le `vencidos`), **F65** (72 cards em baldes `[bulk]` cegos ao radar de dormencia).
