...
*Atualizado: 2026-06-27 ~23:55 (sessão 095) | Ferramenta: Claude Code (Opus 4.8)*
...
- **Indicador Atual:** **4.112** — **41% da meta-prova (10.000)** · 34% do teto (12.000). **s095 = engenharia + cards (sem questões novas):** forjou os 21 flashcards da s094 (#589-609) + construiu o **sync do cronograma (F1-F4)**. Perf. geral 79,5%.
...
- **Contadores:** **51 resumos** em `resumos/` · **364 erros** em `ipub.db` · **~410 cards ativos** (41 andaimes; #609 último — **21 forjados na s095**) · **taxonomia `UNIQUE(area,tema)`**
- **Cronograma (SSOT `Cronograma.pdf`):** **sync F1-F4 LIVE (s095)** — derivador `tools/cronograma.py` + `core/cronograma/grade.json` (30 sem · 352 tasks · 10218q). Semana 10 ✓ FECHADA. **Próxima = Semana 11** (conteúdo); calendário nominal S13 (~2 sem atrás). Frente ativa: **PRD de Revisão Calibrada** (`docs/plans/s094-revisao-calibrada-PRD.md`) — agora **DESBLOQUEADO** pelo F1.
...
5. **Implementar (s096+):** **PRD de Revisão Calibrada** (`docs/plans/s094-revisao-calibrada-PRD.md`) — agora DESBLOQUEADO (sync F1-F4 feito na s095): `dificuldade` em `taxonomia_cronograma` + `infer_nota()` + fusão `/revisar`+`/refrescar`. **Pré-req:** fix `_find_resumo` (indexar `path.stem.lower()` em `get_topic_context._build_index`). **Pendentes:** Tier-3 (schema de altura), limpeza `[bulk]`/`Geral` da taxonomia, enumeração do Drive, re-drill dos bug-nº1 (426, 435, 84, 78, 70, 104), sessão dedicada de Cirurgia, integrar `/schedule` no calendário.
