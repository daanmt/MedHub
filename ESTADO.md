...
| **Checagem de performance** | **`tools/performance.py`** |
...
**2026-04-23 | Claude Code (sessão 071):** **Análise de Úlceras Genitais + skill `/performance`.** *Parte 1:* Bloco de 43 questões com 38 acertos (88,4%) registrado via `registrar_sessao_bulk.py`. Análise cirúrgica dos 5 erros (IDs DB 375, 377, 379, 381, 383) em `Úlceras Genitais.md`. Três eixos de falha metacognitiva identificados: (a) ancoragem em sinal isolado (Q1 — orifício único vs bico de regador); (b) leitura superficial de alternativa (Q2 — "mesmo com lesões herpéticas" deveria ter caído instantaneamente; Q5 — Trichomonas como intruso em lista de úlcera); (c) contaminação cruzada de cortes temporais e checklists incompletos (Q3 — omitiu doxi em lesão >4 semanas; Q4 — misturou 28s/via parto com 36s/início supressão). Resumo enriquecido com 10 novas armadilhas cumulativas. Padrões 6-8 acrescentados em `feedback_analise_questoes.md` (agora 8 padrões totais). *Parte 2:* Criação da skill **`/performance`** via ciclo vibeflow completo (`/discover` → `/gen-spec` → `/implement` → `/audit` com verdict PASS). Script `tools/performance.py` (stdlib-only, read-only) com 5 blocos markdown: total, meta do mês + ritmo, custo/Q (acumulado+mês com faixa visual 🟢🟡🟠🔴🟣), marcos ENARE/Final e áreas fracas+gaps. `METAS_MENSAIS` hardcoded no topo do script. Fix de encoding (cp1252 × emoji no Windows) via `sys.stdout.reconfigure('utf-8')` registrado como convenção arquitetural em `.vibeflow/decisions.md`. Findings de dados expostos pelo relatório (drift Obstetrícia/Obstetricia + `data_sessao` da migração 067) registrados como tech debt.
