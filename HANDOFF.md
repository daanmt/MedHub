...
> 🔴 **s201 EM CURSO (o /ai-eng monitora; veredito e presenca dele no `history/exchange-log.jsonl`):** ✅ (1) ledger F133/F134/F135 (`e0e1ea1`); ✅ (2) F134 teste de PERTURBACAO + F133 teste de PROPRIEDADE (chaves lidas do template), os dois provados VERMELHOS contra o codigo de `da62e95^`; suite verde; (3) #5 lista fechada de objetivo vira `core/objetivos.json`, lida pelo brief e por `solucao_v2_problemas` ('outro:' so com prefixo); (4) #6 teste do render da cadeia no repo (node, skip sem node, 1 golden por estado + PROVISORIA); (5) #7 'discordo/em parte' -> o principal le a solucao, +2 aleatorias por lista re-cunhada, 'lidas/total' no log; #8 estado por elo em 1 portador (o brief), os outros apontam; #4 `analises/*` = autoridade do diagnostico, `o_que_faltou` derivado (sem CLI nova); #9 re-base datado 127->134 nomeando as 7 ou fechar as 7; (6) **caminho PDF** (decisao do operador: ele exporta as listas do EMED em PDF, como os simulados; Chrome fora do fluxo) -> `questoes/*.json` -> `--ingerir`, com o DoD do /ai-eng: golden = 1 lista real com `--expect N`; propriedade = gabarito + 4-5 alternativas, num unico, escopo publico; perturbado = gabarito faltando ou discursiva -> recusa nomeada sem gravar metade; timeout no parser. Depois do PDF: revogar o Chrome em `/banco-emed` e `docs/MISSAO-CHROME-EMED.md` pelos 3 passos do F90 (F135). **Gates do OPERADOR (1 pedido so):** worktrees orfaos (remove + prune + branch) e residual do conteudo do professor em `tmp/`. Pendente sem pressa: (ii) advisories lote5/lote6 -- reconciliar lendo. **Estudo:** proxima lista = t49 Hernias (21q) ou t40 DMG (33q), ambas em v2 no hub; lista resolvida segue o rito do tique em `/banco-emed` (registro -> conclusao no plano -> `--erros` com a Leitura -> racional declarado ANTES do diagnostico -> `insert_questao --errors-file` com `--sessao`/`--emed` -> `analises/*` com `quebrou` 0-based, sem repetir a cadeia). Re-cunhar em v2 a lista que entrar na vez (8 ainda v1: t1 t65 t68 t100 t3 t61 t651 t141; tema novo = lista fechada ANTES do subagente). t40 Q5/Q6/Q18/Q21/Q22 sem tabela na captura; gabaritos suspeitos t100 Q8, t68 Q13, t1 Q21, t65 Q18.
