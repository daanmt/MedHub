...
- 🆕 **PRD `plano-ssot-e-cards-v2` (s183, permit do usuario em 16/09 -- *"planejamento mais estavel, orquestrado por voce"*):** 6 partes em ordem P6 Autopsia diaria -> P5 poda (lote 1 = **125 aposentados sem revlog/reforja** de 192; 67 com historico ficam) -> P1 `plano_tarefas` SSOT + `grade_extensivo.json` -> P2 ledger de listas (`sessoes_bulk.tarefa_id`, `tools/listas.py`) -> P3 painel gerado (Drive deixa de ser SSOT) -> P4 player de cards em Artifact. **Decididas 16/09:** banco e a fonte (Drive congelado); **poda lote 1 EXECUTADA** (125 cards, backup + export `artifacts/backups/pruned_20260916_152847.json`, orfaos 0; aposentados 192 -> 67); player desktop-first. **P6 entregue** (`analisar-questao.md §3.3`), **P5 entregue** (`cards_prune.py`, suite 627). **PRD refinado via discover + 8 specs via gen-spec (s183):** `.vibeflow/specs/plano-ssot-e-cards-v2-part-{1,2,3,4,6,7,8,9}.md` (5 = poda, feita). **Onda 1 ENTREGUE (commit d9f3bcf): part-1 `grade_extensivo.json` + part-9 player (https://claude.ai/artifact/T53xLp1Wdba6W72Mvdid6Y, lote de 60 do dia). **Part-2 ENTREGUE e SEMEADA** (`plano_tarefas` = 896 linhas; 173 com origem `dashboard` a revisar por area). **Part-3 ENTREGUE** (`--concluir/--cortar/--mover/--revisar-area/--confirmar-area/--pendencia-revisao`; 173 linhas aproximadas em 20 areas a revisar com o usuario). Em curso: parts 4 e 6 em paralelo. Ordem restante: 4 (plano_tarefas + semear) -> 3 (concluir/cortar/mover + revisao por area) -> 4 (day_plan le o plano; contratos v1.3; W8 morre) -> 6 (ledger de listas) -> 7 (painel) -> 8 (congelar Drive).** Cortes do discover: sem botao aposentar no player; sem export Sheet; revisao de status COMPLETA por area (usuario recusou o corte). `.vibeflow/prds/plano-ssot-e-cards-v2.md`.
...
- **Engenharia:** `.gitignore` cobre `*.apkg` e `Medcards 2022/`; **suite 714**; `cards_prune.py` = unico caminho de exclusao de card; ledger 109 ids (+E1, E2, F108 propostos; F38 falso positivo de 13/09 declarado no teste vivo).
