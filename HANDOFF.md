...
> 🔴 **s201 (o /ai-eng monitora; checkpoints no `history/exchange-log.jsonl`):** ordem do veredito FECHADA -- F133/F134 (perturbacao + propriedade, vermelhos contra `da62e95^`), #5 `core/objetivos.json`, #6 `tools/test_hub_render.py`, #7 amostra de leitura, #8 estado por elo so no brief, #4 `analises/*` = autoridade, #9 catraca 127, F136 (suite do HANDOFF medida no pre-commit), F135 (Chrome revogado; caminho PDF). **Aberto:** proposta F137 ao /ai-eng (`spec`: backup de antes de ato destrutivo FIXADO fora do keep-5); figura desenhada de 2025 Q52 e 2026 Q58 quando chegar a vez; formato de lista EMED em PDF espera a 1a amostra dele; (ii) advisories lote5/lote6. **Rito do simulado resolvido:** `--registrar` -> UMA linha `registrar_sessao_bulk --area Simulado` com o acerto por bloco na observacao (como a UERJ 2023) -> `plano.py --concluir` -> `--erros` -> analise; o mapa tematico (spoiler) so na Autopsia. Listas da semana 2: t49 Hernias, t40 DMG (v2 no hub).
...
- **Engenharia:** suite **1173** (s201, medida pelo pre-commit -- F136); caminho PDF `tools/prova_pdf.py`; Solucao v2 em cadeia + `core/objetivos.json` + estados no brief; `test_hub_render.py` (goldens); `backup_db` com argparse e rotacao pelo nome (F137); teto 100/150; hub V4 + aba Listas Questoes | Simulados; `/hub-backend` + `plano.py --concluir --leitura`.
