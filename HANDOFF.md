...
*Atualizado: 2026-09-22 (~23h15) -- **s193 (ENGENHARIA, a 1a metade; o estudo e a 2a)**: parte 2 do hub VERDE (nota do hub grava no relogio da revisao, idempotente, com quarentena), manifesto DIFF no ar (so o novo ou alterado sobe) e a Autopsia UERJ 2023 virou arquivo do hub (Version 2). Nenhuma nota esperava no hub. Proximo: o que ele traz (listas + drill no hub).*
...
> 🔴 **ABRIR POR AQUI (estudo):** (a) `ArtifactData list` em `sessoes/2026-09-22h/notas` NO HUB (0 docs em 22/09 ~23h05) -> notas pelo rito: `--record-lote` dry-run (ler JA GRAVADAS / FORA DE ORDEM / REJEITADAS) -> `--apply --expect N`; gravar o hub ANTES de qualquer `/revisar` no chat (senao a nota do celular sai FORA DE ORDEM). (b) Feedback das listas bloco a bloco: `registrar_sessao_bulk` -> `plano.py --concluir` -> `/analisar-questao` com o racional DELE. (c) **Operador:** abrir a Autopsia na aba Aulas do hub e SO ENTAO apagar a avulsa GjpAqF9HVTTDVRUZLkutyf.
...
1. 🏠 **(s193) Hub = O PADRAO, agora gravavel:** parte 2 (`95bcc6d`) + DIFF (`4b21dfe`) + Autopsia na aba Aulas (Version 2). Nada de player, painel ou aula como artifact avulso. Lote `2026-09-22h` = 220 cards, 0 notas. Publish: `hub.py --build --publicado <listagem colada>` -> ler inteiro SO o que esta em `files` -> publish -> `hub.py --confirmar`. Pendente do hub: parte 4 (drill >= 20 no celular com 1 aula no meio; 2 fechamentos) e o `/vibeflow:audit` formal junto dela.
...
3. 🃏 **Avulsos superados pelo hub:** player P8qiJD3ACcofhfnkJaYU8Y (a unica nota, card #92, ja gravada) -- as 4 aulas avulsas ele ja apagou em 22/09; a Autopsia avulsa, linha 5. Card **#92** = conteudo clinico INVERTIDO (vinheta de CoAo critica, cianose diferencial atribuida a TGA): marcado para reforja. Aula-base D10 de Acido-Base + Potassio antes de re-drillar #595 #596 #598 #783 #786 #787.
...
- **FSRS:** divida 177 atrasados + 25 p/ hoje -- pool 742 nunca introduzidos (entram <=90/dia). Regua **v2** desde 18/09. Parametros seguem DEFAULT (F114). Lote do hub `2026-09-22h` (220) no ar; nota do hub grava no relogio da revisao desde a s193.
...
- **Engenharia:** suite **1015**; `auto_check` PASSED; hub com `--record-lote` idempotente (part-2) e manifesto DIFF desde a s193; **boot = panorama** (`plano.py --panorama` + Plano do Dia inteiro; contrato de 5 secoes no hook, `test_boot_entrega_o_panorama`). Item 1.10: 127 orfas (catraca).
...
- **Datas & links:** fim da grade 09/10 · **UERJ 01/11** · 🩺 **Autopsia UERJ 2023 = arquivo do HUB (aba Aulas) desde a s193** -- a avulsa GjpAqF9HVTTDVRUZLkutyf sai depois de conferida no hub (⚰️ a XLCeFfczezCANirt5ykdvP ficou stale) · 🃏📚📊 **Cards, aulas-base e painel = abas do HUB** (linha 3). ⚰️ *Avulsos de 22/09, superados pelo hub:* player P8qiJD3ACcofhfnkJaYU8Y (a unica nota, card #92, ja gravada) e as 4 aulas (Vezrca1J, RCPurGRw, DtEYHEok, RtopChfq; apagadas por ele em 22/09) · 🩻 Raio-X UERJ (foto de 18/09): https://claude.ai/artifact/1tCT3CqnSR3Wb2kSRqcESc

## Ultima sessao -- s193 (2026-09-22, ~22h20-23h15) -- ENGENHARIA: HUB parte 2 + DIFF + Autopsia

Detalhe em `history/session_193.md`. (1) Re-boot pelo `/ai-eng` com 4 decisoes dele; 0 notas no hub. (2) Parte 2 `95bcc6d` pelo principal, sem filho: relogio da revisao, idempotencia por igualdade, quarentena; golden batido (revlog 3.071 -> 3.071; releitura de 22/09 = N 0). (3) DIFF `4b21dfe` + semeadura do registro com evidencia (as 6 aulas nao foram relidas). (4) Autopsia = `artifacts/aula-autopsia-uerj-2023.html`, lida inteira por 1 Sonnet (245k tokens), publicada (Version 2). Suite 986 -> 1015.
...
- 🏠 ⚰️ *(v0 na s192; parte 2 e o DIFF na s193)* **MedHub HUB:** segue aberto a parte 4 (medicao) e o candidato v1 do lote no `db` (+ `index.html` so quando o codigo muda). v1a = RD como arquivo derivado de `history/` (antes de 01/11); v1b = colecao `comandos` + `/loop` (depois de 01/11). Achados da s193 esperando decisao do `/ai-eng`: poda travada por doc permanentemente rejeitado; nota do celular FORA DE ORDEM se o chat revisar o card antes. 🔴 **O owner passa por cima das regras do `db`** (conta compartilhada): a fronteira real e o writer.
...
---
*Historico: history/INDEX.md * Macro: ESTADO.md * Sessao: history/session_193.md * Trocas: history/exchange-log.jsonl * Auditoria: docs/MEMORIA-AUDITORIA.md * Selo: `python tools/selo.py`*
