...
*Atualizado: 2026-09-25 (~23h local) -- **s198 (Fable 5.1) EM CURSO: captura EMED RETOMADA** -- sessao nova do Chrome em modo automatico, brief v2 entregue no chat (gravar em `docs/` foi NEGADO pelo classificador: exfiltracao), `control/hub` = FASE 4 = FILA a partir da t100, `m0010` no Canal, loop `/banco-emed` 5 min (job `*/5`). Escopo da s198 = so extracao (o operador ainda nao resolveu lista nenhuma). Herdado da s197: banco de questoes EMED -- 4 listas / 91 questoes ingeridas (Chrome capturou pela UI; classificador de exfiltracao dele exigiu aprovacao manual); **aba QUESTÕES no MedHub HUB (Version 21)**: modo prova (letra + certeza + riscar; gabarito so no fim), navegacao, revisao por questao com resolucao/forum/racional/elo e analise do hub; `tools/emed_banco.py` + `emed_questoes`/`emed_respostas`; skill `/banco-emed`. Bancada = so captura + canal. Hub db: t40 completa; t26/t96/t49 barradas pelo classificador (hub com link).*
...
> 🗄️ **Bancada EMED https://claude.ai/artifact/Q2Cojm89D9JwRyBYmCD5Db** = so captura do Chrome + Canal (rito completo em `/banco-emed`; captura RETOMADA na s198, fila a partir da t100; 44 listas pendentes na Bancada). 🔒 Conteudo EMED so no `ipub.db` e nos dbs fechados ao dono; nunca no git. Loops (`/banco-emed` 5 min, `/hub-backend` 12h) morrem com a sessao.
