...
> 🗄️ **Bancada EMED https://claude.ai/artifact/Q2Cojm89D9JwRyBYmCD5Db** = so captura do Chrome + Canal (rito completo em `/banco-emed`; captura em PAUSA). 🔒 Conteudo EMED so no `ipub.db` e nos dbs fechados ao dono; nunca no git. Loops (`/banco-emed` 5 min, `/hub-backend` 12h) morrem com a sessao.
