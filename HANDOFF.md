...
> 🔴 **s201 EM CURSO (o /ai-eng monitora; veredito e presenca dele no `history/exchange-log.jsonl`):** ✅ (1) ledger F133/F134/F135 (`e0e1ea1`); ✅ (2) F134 teste de PERTURBACAO + F133 teste de PROPRIEDADE (chaves lidas do template), os dois provados VERMELHOS contra o codigo de `da62e95^`; suite verde; ✅ (3) #5 `core/objetivos.json` = portador unico da lista fechada (`cc30c57`); ✅ (4) #6 `tools/test_hub_render.py` (render da cadeia em node, 6 goldens em `tools/goldens/hub_render/`); ✅ (5) #7 `--solucoes` entrega a amostra de leitura (divergentes + 2 aleatorias estaveis; `soluções lidas N/total` no log), #8 estado por elo = SO o brief §Estado por elo (gate de paridade com a pagina), #4 autoridade do diagnostico = `analises/*` (`o_que_faltou` = copia no ato; regra em `/banco-emed`), #9 as 7 orfas fechadas (catraca 134 -> 127 = base); (6) **caminho PDF** (decisao do operador: ele exporta as listas do EMED em PDF, como os simulados; Chrome fora do fluxo) -> `questoes/*.json` -> `--ingerir`, com o DoD do /ai-eng: golden = 1 lista real com `--expect N`; propriedade = gabarito + 4-5 alternativas, num unico, escopo publico; perturbado = gabarito faltando ou discursiva -> recusa nomeada sem gravar metade; timeout no parser. Depois do PDF: revogar o Chrome em `/banco-emed` e `docs/MISSAO-CHROME-EMED.md` pelos 3 passos do F90 (F135). ✅ Gates do operador (worktrees + residual em `tmp/`) FEITOS com OK dele. ✅ **F136:** HANDOFF no commit = suite completa no pre-commit + numero conferido. Pendente sem pressa: (ii) advisories lote5/lote6 -- reconciliar lendo. **Estudo:** proxima lista = t49 Hernias (21q) ou t40 DMG (33q), ambas em v2 no hub; lista resolvida segue o rito do tique em `/banco-emed` (registro -> conclusao no plano -> `--erros` com a Leitura -> racional declarado ANTES do diagnostico -> `insert_questao --errors-file` com `--sessao`/`--emed` -> `analises/*` com `quebrou` 0-based, sem repetir a cadeia). Re-cunhar em v2 a lista que entrar na vez (8 ainda v1: t1 t65 t68 t100 t3 t61 t651 t141; tema novo = lista fechada ANTES do subagente). t40 Q5/Q6/Q18/Q21/Q22 sem tabela na captura; gabaritos suspeitos t100 Q8, t68 Q13, t1 Q21, t65 Q18.
...
- **Engenharia:** suite **1147** (s201, medida pelo pre-commit -- F136); Solucao v2 em cadeia + `objetivo` + `riscadas` (`db.solucao_v2_problemas`, `--por-objetivo`, `leitura_metacognitiva`); teto 100/150 (`day_plan.TETO_BASE`); **Bancada EMED v2 + `emed_banco.py` + `/banco-emed`** (s197); hub V4 (painel pelo leitor do panorama, quadro `core/hub_quadro.json`, `hub.py --precisa-publicar`, timer ativo, tela de fim com agenda); `/hub-backend` + `plano.py --concluir --leitura`.
