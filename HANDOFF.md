...
> 🔴 **s201 (o /ai-eng monitora; checkpoints no `history/exchange-log.jsonl`):** ordem do veredito FECHADA -- F133/F134 (perturbacao + propriedade, vermelhos contra `da62e95^`), #5 `core/objetivos.json`, #6 `tools/test_hub_render.py`, #7 amostra de leitura, #8 estado por elo so no brief, #4 `analises/*` = autoridade, #9 catraca 127, F136 (suite do HANDOFF medida no pre-commit), F135 (Chrome revogado; PDF PROVADO so para provas UERJ -- lista EMED em PDF NAO feita ate a amostra dele virar golden). **Aberto:** F137 spec GO+3 ALTERA -- feito `backup_db.py --fixar` + 1o fixado (`ipub_fixado_20260926_162622_*`, sha256 em `artifacts/backups/FIXADOS.json`); falta o writer destrutivo RECUSAR sem fixado, `--desfixar` com rastro e sha256 no F-item; figura desenhada de 2025 Q52 e 2026 Q58 quando chegar a vez; formato de lista EMED em PDF espera a 1a amostra dele; (ii) advisories lote5/lote6. **Rito do simulado resolvido:** `--registrar` -> UMA linha `registrar_sessao_bulk --area Simulado` com o acerto por bloco na observacao (como a UERJ 2023) -> `plano.py --concluir` -> `--erros` -> analise; o mapa tematico (spoiler) so na Autopsia. Listas da semana 2: t49 Hernias, t40 DMG (v2 no hub).
