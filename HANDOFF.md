...
2. **Recalibrar a S2 pelo acerto por bloco:** gerador NO REPO desde a s189 (`tools/trilha.py`): ajustar `core/cronograma/trilha/parametros.json` (piso/teto) ou `trilha/custom.json`, `python tools/trilha.py` (le o diff) -> `--gravar`, depois `plano.py --semear --dry-run` -> `--apply --expect 0`. So entao abrir o mapa da prova FEITA (`uerj_mapa_questoes_2021-2026.json`) para a Autopsia.
...
- **Cronograma:** `plano_tarefas` = SSOT; **a Fase 1 e ditada por `core/cronograma/plano_trilha.json`** (115 overrides, GERADO por `tools/trilha.py` desde a s189: estrategia em `core/cronograma/trilha/parametros.json`, ajuste de linha em `trilha/custom.json`, depois `--gravar` + `--semear`; o `--mover` e desfeito pelo re-seed). Links das listas: `links_listas.json` (66 -> 381 linhas com link). Incidencia UERJ: `prevalencia_uerj.json`.
