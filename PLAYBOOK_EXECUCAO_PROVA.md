...
| **Enunciado negativo (EXCETO/ERRADA)** | Marca a afirmação **verdadeira** em vez de isolar a falsa (primo de *leitura*, não de verificação); não usa o **nº de F como checksum** (deve ser 1) | s079-080 (3×), s086 (Q4 hipertensivas), s112-118, **s120 (mini-drill: checksum nº de F=1; REINCIDIU em prova real no dia seguinte -- Q2 SUS "NÃO traduz descentralização", marcou ação verdadeira)**, **s138 simulado 3 (3x: melanoma "NÃO é subtipo" marcou extensivo-superficial que É subtipo real; psoríase "NÃO é biológico aprovado" marcou risanquizumabe que É aprovado; BISAP "NÃO é critério" marcou derrame pleural que É critério -- padrão: reconhece um termo familiar de OUTRA categoria/lista e marca como se pertencesse, em vez de rotular V/F cada opção)** |
| **Aborda pela lente da especialidade/etiqueta, ignora a instabilidade clínica geral** | O rótulo diagnóstico (psiquiátrico, obstétrico, neurológico) puxa direto pra conduta *daquela especialidade*, pulando o reconhecimento de que a ameaça imediata é clínica geral (hemodinâmica, metabólica) e vem antes | s138 simulado 3 -- **3 especialidades**: delirium por EHH → foi direto pra ATB empírica sem confirmar a hiperglicemia; Anorexia grave com QT longo → internou em saúde mental em vez de clínica médica; HPP com índice de choque alterado → foi revisar canal de parto antes de reconhecer a instabilidade e ativar o protocolo. Padrão novo, `tools/habilidades.py` #2815 |
...
**Confirmado em 7 especialidades no ledger de habilidades** (`tools/habilidades.py --reincidentes`, **padrão nº1 do ledger inteiro** desde s138) -- s131: SBC-HAS 2025 (nova faixa "pré-hipertensão" 120-139/80-89), Reanimação Neonatal 2026 (sem aspiração traqueal de rotina mesmo com mecônio espesso), Diretriz de Dislipidemia 2025 (meta de LDL <40 em risco extremo/eventos recorrentes), ATLS 11ª edição (xABCDE — controle de hemorragia exsanguinante antes da reposição volêmica); **s138 (simulado 3) somou mais 3**: calendário vacinal PNI (pneumocócica 10-valente → 20-valente aos 2/12 meses), SBD 2024 (TOTG-1h obrigatório em rastreio negativo com ≥3 fatores de risco -- questão com só 15% de acerto geral), esquema de malária (tafenoquina dose única substitui primaquina 7 dias quando G6PD tem atividade alta). **Não é mais um padrão de nicho** -- atravessa Cardio, Neonatologia, Dislipidemia, Trauma, Imunização, Endocrino/Preventiva e Infecto. Vale a pena virar hábito de atualização periódica das diretrizes mais citadas em prova (SBC, MS/PNI, SBD, ATLS), não só reflexo de sala.
...
## Evidência da s138 — simulado ENARE/ENAMED 3 (100q, 60%), +6pp sobre o simulado 2

Segundo simulado completo pós-virada multi-banca, analisado retroativamente (prova em 06/08, análise completa em 07/08 via reconciliação de sessão). 40 erros, todos cadastrados com análise completa (`questoes_erros` 719-758):

- **Conhecimento desatualizado dispara para o topo do ledger** (ver seção própria): 3 novas especialidades somadas às 4 da s131 -- 7 no total, virou o padrão nº1 do ledger inteiro (`tools/habilidades.py --reincidentes`), à frente até do "achado normal exclui".
- **Enunciado negativo reincidiu 3x** (ver tabela acima) -- todas em questões de decoreba densa (subtipos de melanoma, biológicos aprovados, critérios do BISAP), não em vinhetas clínicas longas.
- **Padrão novo: aborda pela especialidade, ignora a instabilidade geral** (ver tabela acima) -- 3 especialidades diferentes convergindo na mesma falha de sequenciamento (rótulo diagnóstico antes de reconhecer gravidade).
- **Reincidência direta confirmada:** erro #729 (toxoplasmose IgM+/IgG- isolado, pular pra esquema tríplice) é o **mesmo elo exato** do erro #626 do simulado 2 (pular pra teste de avidez) -- a lição "repetir sorologia antes de qualquer conduta definitiva" não gravou depois da primeira exposição.
- **2 novas instâncias do discriminador-que-exclui** (família do padrão-mestre `feedback_bug_discriminador_exclui`): bypass gástrico em Y de Roux exclui CPRE convencional (colangite); ausência de irritação peritoneal exclui laparotomia imediata (volvo de sigmoide). E uma terceira fora da família cirúrgica: toque vaginal é contraindicação absoluta em suspeita de placenta prévia, não um passo de rotina.
- **Raio-X por área:** Obstetrícia (7 erros) e Pediatria (6) concentram quase 1/3 dos 40 -- não por lacuna de conteúdo isolada, mas puxado pelos padrões transversais acima (diretriz desatualizada, enunciado negativo, discriminador-exclui aparecem em ambas as áreas).
