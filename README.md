...
Two curves run in parallel. The **card-level** curve is FSRS, described above. The **theme-level** forgetting curve is separate: `tools/review_radar.py` ranks dormant themes, `tools/dormant_refresh.py --stamp --kind {dormant_refresh,directed_review}` records a thematic review in `review_log`, and the hard boundary is that this path never touches FSRS. The `/revisar` skill is the single review competence, in two phases: DRENAR (the card-by-card FSRS player, and the only surface that writes FSRS) and a closing DIRECTED REVIEW (the session's only teaching surface, aimed at the themes that scored 1-2; FSRS read-only, stamps `review_log`).
