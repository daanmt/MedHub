...
## Cláusula 15 -- o player mora no MedHub HUB, um artifact só (s192, v1.7; v1.8 na s193)
...
- **Relógio da revisão (em vigor desde a s193, `medhub-hub-v0-part-2`).** O `--record-lote` grava cada nota no momento em que ela foi dada (o `ts` da página, em hora local, truncado ao segundo), não na hora da gravação: é o que o FSRS modela, e é o que torna a gravação idempotente por igualdade. Revisão mais nova sem a igual = FORA DE ORDEM, reportada, nunca gravada por cima; o writer (`record_review(quando=...)`) recusa revisão no futuro ou não posterior à última do card. ⚰️ *O gate "notas do hub não são gravadas" (s192) foi cumprido na s193 sem usar o fallback.*  <!-- CHECK: test_duas_notas_do_mesmo_card_em_duas_sessoes_gravam_nos_seus_ts -->
- **Quarentena no writer.** A conta é compartilhada, quem abre é OWNER, e o owner atende todo nível das regras do `db` -- a regra (`sessoes` = `interact`, o resto = `admin`) limita membro da organização, não o owner. Por isso a fronteira real é o CLI: doc estranho é rejeitado e reportado, e os válidos seguem (em vigor desde a s193; até ela, o doc estranho recusava o lote inteiro).  <!-- CHECK: test_quarentena_grava_os_validos_e_reporta_cada_doc_estranho -->
- **Poda (v1.8).** O `db` de um artifact guarda no máximo 5.000 documentos (medido no servidor em 22/09: "1 of 5000 documents used") e a página grava 1 por card: a sessão que SAIU da aba é podada depois de uma releitura dar **0 válidas não gravadas** e de as rejeitadas e as FORA DE ORDEM estarem **arquivadas inteiras**, com o motivo, em `history/quarentena/<sessao>.json` e commitadas. Invariante: nada sai do `db` sem cópia no SSOT -- um doc permanentemente estranho pode ser uma revisão real com campo quebrado, e o arquivo preserva a recuperação manual; deixá-lo no `db` travaria a poda e comeria o teto. A sessão do lote corrente nunca é podada.  <!-- CHECK: test_rejeitada_e_fora_de_ordem_vao_inteiras_para_a_quarentena_no_apply -->
  ⚰️ *Revogada na v1.8 (s193): a condição de releitura com "0 novas e 0 rejeitadas" (s192) -- um doc permanentemente estranho travava a poda para sempre.*
- **O chat abre gravando o hub (v1.8, decisão do `/ai-eng` na s193).** O `/revisar` no chat começa pelo `--record-lote --apply --expect N` das notas pendentes do hub (N pode ser 0), e a linha entra no session log antes da 1a revisão do chat: o chat nunca passa na frente do celular para o mesmo card. A fila (`--next`/`--list`/`--export-player`) avisa em stderr quando a última gravação do hub passou de 6h; o FORA DE ORDEM segue como detector de violação, e a nota perdida vai para a quarentena.  <!-- CHECK: test_fila_do_chat_avisa_quando_o_hub_nao_foi_gravado -->
