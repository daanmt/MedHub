...
| `retire_heuristic_cards.py` | Bankruptcy of the 70 legacy heuristic flashcards (`needs_qualitative = 1 → 2`), retiring them from the `/revisar` queue instead of regenerating them. Supersedes the backfill-regen path in `estilo-flashcard.md` for these cards. FSRS focus moves to the 332 agent-crafted qualitative cards. Backup-first, dry-run flag. Policy in `core/contracts/fsrs-management-contract.md`. | Sessão 075 |
